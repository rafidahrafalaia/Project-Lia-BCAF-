...
declare var $: any;
@Component({
  selector: 'app-approval-archive',
  templateUrl: './approval-archive.component.html',
  styleUrls: ['./approval-archive.component.css']
})
export class ApprovalArchiveComponent implements OnInit {
  public search:any = '';
  locked: any[] = [];
  userData : any;
  code:String;
  ...
  fetchData() {
    ...
    this.api.getKontrakArchiveApproval().subscribe((d) => {
      this.temp = d;
      console.log(d);
      this.rows = d;
    });
  ...
  saveKontrak(nomor){
    ...
    this.api.updateKontrakDetail({id:this.kontrak.id,nomor:nomor,jenisSGU:this.kontrak2.jenisSGU,spHak:this.kontrak2.spHak,faktur:this.kontrak2.faktur,polis:this.kontrak2.polis,nama:this.kontrak.nama,
  ...
  selectedRow(nomor){
    console.log(nomor) 
    this.api.updateKontrak({ id:this.kontrak.id,nomor:nomor, user:this.userData.name,approval:true,recive:true}).subscribe((kontrak)=>{
