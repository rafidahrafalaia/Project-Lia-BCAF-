...
declare var $: any;
@Component({
  selector: 'app-approval-docust',
  templateUrl: './approval-docust.component.html',
  styleUrls: ['./approval-docust.component.css']
})
export class ApprovalDocustComponent implements OnInit {
  public search:any = '';
  locked: any[] = [];
  userData : any;
  code:String;
  ...
  fetchData() {
    ...
    this.api.getKontrakDocustApproval().subscribe((d) => {
      this.temp = d;
      console.log(d);
      this.rows = d;
    });
  ...
  selectedRow(nomor){
    console.log(nomor) 
    this.api.updateKontrak({ id:this.kontrak.id,nomor:nomor, approval:true,recive:true}).subscribe((kontrak)=>{
