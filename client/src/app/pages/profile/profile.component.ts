...
export class ProfileComponent implements OnInit {
  ...
  fetchData() {
    ...
    this.api.getKontrakArchiveRelease().subscribe((d) => {
      this.temp = d;
      console.log(d);
      this.rows = d;
    });
  ...
  exportAsXLSX():void {
    this.excelService.exportAsExcelFile(this.temp, 'release_archive');
  }
  ...
  saveKontrak(nomor){
    // let { agencyId } = this.auth.decodeJWT();
    console.log('edit',this.kontrak.id,this.kontrak.nama, nomor,this.kontrak.jenisSGU,this.kontrak.buktiBayar,this.kontrak2.jadwalSGU,this.kontrak.tglRealisasi,this.kontrak2.pelunasan)
  ...
  selectedRow(nomor){
    console.log(nomor) 
    this.api.updateKontrak({ id:this.kontrak.id,nomor:nomor, recive:false,user:this.userData.name,approval:true}).subscribe((kontrak)=>{
