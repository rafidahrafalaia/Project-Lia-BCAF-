...
@Component({
  selector: 'app-borrow-archive',
  templateUrl: './borrow-archive.component.html',
  styleUrls: ['./borrow-archive.component.css']
})
export class BorrowArchiveComponent implements OnInit {
  collapsed = true;
  ...
  saveKontrak(nomor){
    ...
    this.api.updateKontrakDetail({id:this.kontrak.id,nomor:nomor,jenisSGU:this.kontrak2.jenisSGU,po2:this.kontrak2.po2,sp_Bpkb:this.kontrak2.sp_Bpkb,jadwalSGU:this.kontrak2.jadwalSGU,nama:this.kontrak.nama,
  ...
  selectedRow(nomor){
    console.log(nomor) 
    this.api.setBorrow({ id:this.kontrak.id,nomor:nomor,recive:true, pengajuan:true}).subscribe((kontrak)=>{
