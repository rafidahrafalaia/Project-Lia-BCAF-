...
declare var $: any;
@Component({
  selector: 'app-borrow-all',
  templateUrl: './borrow-all.component.html',
  styleUrls: ['./borrow-all.component.css']
})
export class BorrowAllComponent implements OnInit {
  public search:any = '';
  locked: any[] = [];
  userData : any;
  code:String;
  ...
  fetchData() {
    ...
    this.api.getKontrakBorrowAll().subscribe((d) => {
      this.temp = d;
      console.log(d);
      this.rows = d;
    });
  ...
  selectedRow(nomor){
    ...
    // this.api.updateKontrak({ id:this.kontrak.id,nomor:nomor, recive:true,user:this.userData.name,approval:true}).subscribe((kontrak)=>{
      this.api.setBorrow({ id:this.kontrak.id,nomor:nomor,borrow:true}).subscribe((kontrak)=>{
